...
mydatalong <- mydata %>%
  mutate_each(funs(normalit), which(sapply(., is.numeric))) %>%   # Normalize eahc numeric column
...
mydatalong %>%
  ...
  ggplot(aes(variable, value, group=FileName, colour=genotype)) + 
  geom_line() + 
...
mydatalong %>%
  filter(Time == max(Time)) %>%   # Select just one time point (the last)
  ggplot(aes(variable, value, group=FileName, colour=genotype)) +   geom_line() + 
  facet_wrap(~genotype, nrow=2)+ 
  theme(text = element_text(size=9))
```
...
mydatalong %>%
  filter(Time == max(Time)) %>%   # Select just one time point (the last)
  ggplot(aes(variable, value, group=FileName, colour=genotype)) +   geom_line() + 
  coord_polar() +  # Change the coordinate system
  facet_wrap(~genotype, nrow=2)+ 
  theme(text = element_text(size=9))
...
mydatalong %>%
  filter(Time == max(Time)) %>%   # Select just one time point (the last)
  ggplot(aes(variable, value, group=FileName, colour=genotype)) +   
  geom_polygon(fill=NA) +  # Close the radar plot
  coord_polar() +  # Change the coordinate system
  facet_wrap(~genotype, nrow=2)+ 
  theme(text = element_text(size=9))
...
pl <- mydatalong %>%
  ggplot(aes(variable, value, group=FileName, colour=genotype, frame=Time)) +
  geom_polygon(fill=NA) +  # Close the radar plot
  coord_polar() +  # Change the coordinate system
  facet_wrap(~genotype, nrow=2)+ 
  theme(text = element_text(size=9))
